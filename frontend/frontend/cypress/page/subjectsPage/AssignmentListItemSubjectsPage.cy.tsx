...
describe('AssignmentListItemSubjectsPage', () => {
    ...
    it('renders', () => {
        ...
        cy.get(`#projectName`).should('exist').should('contain', mockProps.projectName);
        cy.get('#deadline').should('exist').should('contain',  dayjs(mockProps.dueDate).format('DD/MM/YYYY HH:mm'));
        cy.get('#score').should('exist').should('contain', mockProps.score.score + '/' + mockProps.maxScore);
        cy.get('#submissions').should('exist').should('contain', mockProps.submissions);
    });
