...
function fakeFetch(date: Dayjs, {signal}: { signal: AbortSignal }, deadlines: Dayjs[]) {
    return new Promise<{ deadlinesToDisplay: Dayjs[] }>((resolve, reject) => {
        const timeout = setTimeout(() => {
            const deadlinesToDisplay = deadlines.filter(deadline =>
                deadline.month() === date.month() && deadline.year() === date.year()
            );
...
function ServerDay(props: PickersDayProps<Dayjs> & { highlightedDays?: number[] }) {
    ...
    return (
        <Badge
            ...
            overlap="circular"
            badgeContent={isSelected ? <AssignmentIcon color={"primary"} fontSize={"small"}/> : undefined}
        >
    ...
    const fetchHighlightedDays = (date: Dayjs) => {
        ...
        fakeFetch(date, {
            ...
            .then(({deadlinesToDisplay}) => {
                setHighlightedDays(deadlinesToDisplay.map(deadline => deadline.date()));
                setIsLoading(false);
            })
            .catch((error: Error) => {
