...
class IndieningBestandListViewTest(TestCase):
    def setUp(self):
        ...
        self.teacher = GebruikerFactory.create(is_lesgever=True)
        self.student = GebruikerFactory.create(is_lesgever=False)
        self.indiening_bestand1 = IndieningBestandFactory.create()
        self.indiening_bestand2 = IndieningBestandFactory.create()
        self.indiening_bestand2.indiening.groep.studenten.add(self.student)
        self.client.force_login(self.teacher.user)
        self.url = reverse("indiening_bestand_list")

    def test_indiening_bestand_list_get_as_teacher(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)
...
class IndieningBestandDetailViewTest(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.teacher = GebruikerFactory.create(is_lesgever=True)
        self.student = GebruikerFactory.create(is_lesgever=False)
