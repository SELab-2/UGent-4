...
class VakSerializerTest(APITestCase):
    ...
    def test_contains_expected_fields(self):
        ...
        self.assertCountEqual(data.keys(), ["vak_id", "name", "students", "teachers"]) # geen projects hier?
    ...
    def test_students_field_content(self):
        data = self.serializer.data
        self.assertCountEqual(data["students"], [student.pk for student in self.students])

    def test_teachers_field_content(self):
        data = self.serializer.data
        self.assertCountEqual(data["teachers"], [teacher.pk for teacher in self.teachers])
